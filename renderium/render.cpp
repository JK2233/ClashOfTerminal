...
namespace render
{
    enum ScreenObjectType : uint8_t
    {
        ...
        //renders a Texture but only within a set Ellipse
        e_EllipseMaskedTexture,
    ...
    std::string getNameOfScreenObjectType(ScreenObjectType type) {
        ...
        case e_SquareMaskedTexture:
            ...
            break;
        case e_EllipseMaskedTexture:
            ret = "EllipseMaskedTexture";
            break;
    ...
    namespace internal {
        ...
        inline void renderScreenObject(int_fast16_t idx) {
            ...
            case e_Label:
            {
                if (InternalTextStreams.size() <= objectresorceIDBuffer[idx])
                {
                    ReportError("This string does not exist", true, true);
                    break;
                }
                for (int_fast32_t i = 0; i < InternalTextStreams[objectresorceIDBuffer[idx]].size(); ++i) {
                    ...
                    //if (true)
                {
                    ...
                    if (mainThreadFinnishedDataTransfer == true)
                    {
                        ...
                        PhysicsFrameCount++;
                        {
                            ...
                            for (int_fast16_t i = 0; i < TextStreams.size(); i++)
                            {
                                InternalTextStreams.push_back(TextStreams[i]);
                            }
                        }
    ...
    void AddLabel(std::u32string _text, int16_t _posY, int16_t _posX, uint8_t _forColor) {
        ScreenObject tmpObject;
